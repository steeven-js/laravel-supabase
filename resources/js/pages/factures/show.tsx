...
export default function FactureShow({ facture, madinia, pdfStatus: initialPdfStatus }: Props) {
    ...
    useEffect(() => {
        if (!initialPdfStatus) {
            const fetchPdfStatus = async () => {
                try {
                    const response = await fetch(`/factures/${facture.id}/pdf-status`);
                    if (response.ok) {
